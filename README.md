...
새로운 팀에 합류해 처음 Git을 마주할 때의 막막함을 기억하나요? '꼬깃(GGoGit)'은 터미널의 까만 화면이 두려운 신규 부원들을 위해 만들어진 **인터랙티브 Git 시각화 웹 게임**입니다. 명령어를 칠 때마다 즉각적으로 반응하는 시각화된 트리를 통해, 코드를 날려먹고 에러를 내는 모든 삽질의 과정이 결국 훌륭한 성장의 기록(History)으로 남는다는 것을 직관적으로 배울 수 있습니다.
